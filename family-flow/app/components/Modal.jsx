...
export default function Modal({ open, onClose, onSubmit, mode, event }) {
	const [eventType, seteventType] = useState(mode === "edit" && event ? event.eventType : "meal");
	const [title, setTitle] = useState(mode === "edit" && event ? event.title : "");
	const [activityCategory, setActivityCategory] = useState(mode === "edit" && event ? event.activityCategory : "");
	const [startTime, setStartTime] = useState(mode === "edit" && event ? event.startTime : "");
	const [date, setDate] = useState(mode === "edit" && event ? event.date : "");
	const [membersId, setmembersId] = useState(mode === "edit" && event ? event.membersId : []);
		...
		<Dialog
			...
			<DialogContent>
				<Box component='form' onSubmit={handleSubmit}>
					...
					<ToggleButtonGroup
						...
						<ToggleButton value='activity' color='secondary' sx={{ borderRadius: 2 }}>
					...
					<TextField
						placeholder={eventType === "meal" ? "e.g., Spaghetti Bolognese" : "e.g., Park Playdate"}
						fullWidth
						required
						margin='dense'
						value={title}
			...
			<DialogActions
				...
				<Button
					...
					{mode === "add" ? (eventType === "meal" ? "Add Meal" : "Add Activity") : "Save Changes"}
				</Button>
			</DialogActions>
		</Dialog>
	);
